...
const COPY = {
  ...
  ms: {
    ...
    factCommunity: "16 Bahasa Inggeris · kohort Mandarin",
...
const PATHWAYS = {
  en: [
    ...
    ["04", "CMI recognition", "What awarded and endorsed means for this professional programme.", "/executive-mba#recognition"],
  ...
  ms: [
    ...
    ["04", "Pengiktirafan CMI", "Maksud dianugerahkan dan disokong bagi program profesional ini.", "/ms/executive-mba#recognition"],
  ...
  zh: [
    ...
    ["04", "CMI 专业认可", "了解本专业课程获颁授并背书的具体含义。", "/zh/executive-mba#recognition"],
      ...
      <section className="commerce-section commerce-experience" aria-labelledby="commerce-experience-title">
        <div className="wrap">
          ...
          <div className="commerce-experience-grid">
            <Reveal><figure className="commerce-experience-main"><Image src="/media/future-commerce/workshop-system.webp" alt={copy.experienceLabels[0]} width={1920} height={1440} sizes="(max-width: 820px) 100vw, 62vw" /><figcaption><span className="mono">01</span><strong>{copy.experienceLabels[0]}</strong></figcaption></figure></Reveal>
            <Reveal delay={55}><figure><Image src="/media/future-commerce/founder-atelier.webp" alt={copy.experienceLabels[1]} width={1920} height={1080} sizes="(max-width: 820px) 100vw, 34vw" /><figcaption><span className="mono">02</span><strong>{copy.experienceLabels[1]}</strong></figcaption></figure></Reveal>
            <Reveal delay={90}><figure><Image src="/media/future-commerce/community-commencement.webp" alt={copy.experienceLabels[2]} width={1920} height={1440} sizes="(max-width: 820px) 100vw, 34vw" /><figcaption><span className="mono">03</span><strong>{copy.experienceLabels[2]}</strong></figcaption></figure></Reveal>
          </div>
          <div className="commerce-experience-summary">
            <Reveal>
              <div className="commerce-audience-summary">
                <p className="mono">{copy.audienceKicker}</p>
            ...
            <div className="commerce-shift-list" aria-label={copy.outcomesKicker}>
              <header><p className="mono">{copy.outcomesKicker}</p><h3>{copy.outcomesTitle}</h3><p>{copy.outcomesIntro}</p></header>
              {OUTCOMES[locale].map(([from, to], index) => (
                <Reveal key={from} delay={index * 45}><article><span className="mono">0{index + 1}</span><div><small>{from}</small><strong>{to}</strong></div></article></Reveal>
              ))}
            </div>
          </div>
        </div>
      ...
      <section id="recognition" className="commerce-section commerce-recognition">
        <div className="wrap">
          ...
          <div className="commerce-trust-marks" aria-label={locale === "zh" ? "课程机构与认可" : locale === "ms" ? "Organisasi dan pengiktirafan program" : "Programme organizations and recognition"}>
            <div><Image src="/brand/abc-mark.webp" alt="Asian Business Consulting" width={650} height={180} /><p><strong>{locale === "zh" ? "课程设计与授课" : locale === "ms" ? "Reka bentuk dan penyampaian" : "Programme design and delivery"}</strong>Asian Business Consulting</p></div>
          ...
          <div className="commerce-proof-gallery">
            <figure><Image src="/brand/community/graduation-cohort.webp" alt="Future Ready Executive MBA graduation cohort" width={894} height={596} sizes="(max-width: 760px) 100vw, 58vw" /><figcaption>{PROGRAMME_PROOF.graduationAttendance} {locale === "zh" ? "名毕业生出席首届毕业典礼。" : locale === "ms" ? "graduan menghadiri majlis graduasi pertama." : "graduates attended the inaugural graduation."}</figcaption></figure>
            <figure><Image src="/brand/community/faculty-event.webp" alt="Future Ready Executive MBA faculty and programme representatives" width={2560} height={1440} sizes="(max-width: 760px) 100vw, 38vw" /><figcaption>{locale === "zh" ? "实践型导师与课程代表。" : locale === "ms" ? "Fakulti berorientasikan amalan dan wakil program." : "Practitioner-led faculty and programme representatives."}</figcaption></figure>
          </div>
        </div>
      </section>
