...
export const metadata = withSeo("/ms/resources/advancement-brief", {
  ...
  description: "Panduan program yang boleh dicetak: untuk siapa program ini, struktur tiga bulan, kaedah aplikasi, yuran peserta Malaysia, pengiktirafan CMI dan langkah seterusnya.",
});
  ...
  return (
    <div className="brief-shell">
      ...
      <article className="brief-document">
        ...
        <section className="brief-chapter"><span className="mono">02 / Kaedah</span><div><h2>Aplikasikan inkuiri berstruktur terhadap situasi sebenar.</h2><p>Tiga bulan ini merangkumi enam hari latihan, tiga sesi bulanan, rangka kerja pengamal, diagnostik, bimbingan dan projek aplikasi perniagaan. Fokus pembelajaran meliputi sistem, bukti, andaian, pilihan dan pelaksanaan.</p><div className="brief-facts"><div><strong>6 hari</strong><span>Latihan program</span></div><div><strong>3 sesi</strong><span>Diatur setiap bulan</span></div><div><strong>12 modul</strong><span>Rangka kerja aplikasi</span></div></div></div></section>
        <section className="brief-chapter"><span className="mono">03 / Susunan tiga bulan</span><div><h2>Tiga bulan, program yang lengkap.</h2><p>Peserta menamatkan tiga sesi bulanan, bimbingan dan projek aplikasi perniagaan dalam tempoh tiga bulan. Peserta yang memenuhi keperluan program menerima CMI Certificate of Recognition.</p><p>Chartered Manager ialah laluan CMI berasingan yang bersifat pilihan, dengan kelayakan, penilaian, keahlian dan yuran tersendiri; ia tidak termasuk dalam program ini atau yuran yang diterbitkan.</p></div></section>
        ...
        <section className="brief-chapter"><span className="mono">05 / Yuran</span><div><h2>Yuran standard dan kelayakan biasiswa.</h2><div className="brief-facts"><div><strong>{FACTS.priceStd}</strong><span>Yuran standard program</span></div><div><strong>− {FACTS.scholarshipAmount}</strong><span>Biasiswa untuk pemohon Malaysia yang layak</span></div><div><strong>{FACTS.priceAfterScholarship}</strong><span>Dibayar hanya selepas biasiswa diluluskan secara bertulis</span></div></div><p>Biasiswa tertakluk pada kekosongan, penilaian dan kelulusan bertulis; ia tidak diberikan secara automatik. {HRD_CORP_CLAIM_MS}</p></div></section>
