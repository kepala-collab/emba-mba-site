...
export const metadata = withSeo("/zh/lp/google", {
  title: `马来西亚 Executive MBA — 英国 CMI 认可，${FACTS.durationMonths} 个月完成`,
  description:
    `${FACTS.durationMonths} 个月修完 Future Ready Executive MBA 专业发展课程：${FACTS.trainingDays} 个培训日、${FACTS.liveSessions} 次导师带领的研习课，以及真实企业应用项目。雇主资助须符合 HRD Corp 条件并获批准。`,
...
const INCLUDED = [
  [`${FACTS.liveSessions} 次研习课，导师全程带领`, "由 ABC 在班次说明中列明的企业实战者、顾问与高管教练亲自主讲。"],
...
const CMP = [
  ["时间", `${FACTS.durationMonths} 个月完成：${FACTS.trainingDays} 个培训日，分 ${FACTS.liveSessions} 次研习课修读`, "18–24 个月"],
  ["投资", `标准费用 ${FACTS.priceStd}；名额有限，仅择优授予符合资格的马来西亚申请者，须经评估与书面批准。奖学金金额与最终应付费用均以书面个别确认`, "以授予院校公布的学费及相关费用为准"],
  ["形式", `证书阶段共 ${FACTS.liveSessions} 次指定的周末研习课，学员照常在职工作`, "按授予院校公布的学术课程时间表修读"],
  ["考核", "以学员自身企业为对象的企业应用项目，不设传统考试或论文", "作业或考试，另加论文"],
            ...
            <div>
              ...
              <Reveal delay={110}>
                <p style={{ color: "var(--ink-2)", fontSize: "1.1rem", maxWidth: "44ch", margin: "22px 0 26px", lineHeight: 1.9 }}>
                  照常在职工作的同时，{FACTS.durationMonths} 个月内修完 {FACTS.liveSessions} 次研习课、企业应用项目与这门 CMI 认可的专业发展课程。<b style={{ color: "var(--ink)" }}>Chartered Manager</b> 属独立可选的 CMI 路线，另有独立的资格、评估与费用。
                </p>
              </Reveal>
              <Reveal delay={150}>
              ...
              <Reveal delay={190}>
                <div className="lpg-stats" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 1, background: "var(--line)", marginTop: 24, borderRadius: 12, overflow: "hidden" }}>
                  {[[`${FACTS.durationMonths} 个月`, "完整专业发展课程"], [`${FACTS.trainingDays} 天`, `分 ${FACTS.liveSessions} 次研习课修读`], ["资格评估", "马来西亚申请者择优评估奖学金"]].map(([b, s]) => (
