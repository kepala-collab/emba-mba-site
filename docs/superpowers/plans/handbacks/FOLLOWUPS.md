...
- [ ] ExecutiveDiagnostic result names the PDF "Progression Guide" in all three languages; it must be "the 2026 programme guide" (spec section 10 naming). Fix in ExecutiveDiagnostic.tsx en/ms/zh.
- [ ] A shared surface repeats "Free" more than once (see R2-components-review-zh.json); reduce to at most one fine-print "Free PDF" per surface in all three languages.
